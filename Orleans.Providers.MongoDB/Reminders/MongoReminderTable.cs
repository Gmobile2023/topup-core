...
public sealed class MongoReminderTable : IReminderTable
{
        ...
        IGrainReferenceConverter grainReferenceConverter)
    {
        ...
        serviceId = clusterOptions.Value.ServiceId ?? string.Empty;
    ...
    {
        return DoAndLog(nameof(ReadRows), () => { return collection.ReadRow(key); });
    }

    /// <inheritdoc />
    public Task<bool> RemoveRow(GrainReference grainRef, string reminderName, string eTag)
    {
        return DoAndLog(nameof(RemoveRow), () => { return collection.RemoveRow(grainRef, reminderName, eTag); });
    }

    /// <inheritdoc />
    public Task<ReminderEntry> ReadRow(GrainReference grainRef, string reminderName)
    {
        return DoAndLog(nameof(ReadRow), () => { return collection.ReadRow(grainRef, reminderName); });
    }

    /// <inheritdoc />
    public Task TestOnlyClearTable()
    {
        return DoAndLog(nameof(TestOnlyClearTable), () => { return collection.RemoveRows(); });
    }

    /// <inheritdoc />
    public Task<string> UpsertRow(ReminderEntry entry)
    {
        return DoAndLog(nameof(UpsertRow), () => { return collection.UpsertRow(entry); });
    }
    ...
    private async Task<T> DoAndLog<T>(string actionName, Func<Task<T>> action)
    {
        ...
        {
            logger.LogError((int) MongoProviderErrorCode.Reminders_Operations, ex,
                $"ReminderTable.{actionName} failed. Exception={ex.Message}");
