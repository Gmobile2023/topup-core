...
public static class JsonBsonConverter
{
    public static BsonDocument ToBson(this JObject source)
    {
        ...
        foreach (var property in source)
            result.Add(property.Key.EscapeJson(), BsonExtensionMethods.ToBson(property.Value));
    ...
    public static JObject ToJToken(this BsonDocument source)
    {
        ...
        foreach (var property in source) result.Add(property.Name.UnescapeBson(), property.Value.ToJToken());
    ...
    public static BsonArray ToBson(this JArray source)
    {
        ...
        foreach (var item in source) result.Add(BsonExtensionMethods.ToBson(item));
    ...
    public static JArray ToJToken(this BsonArray source)
    {
        ...
        foreach (var item in source) result.Add(item.ToJToken());
    ...
    private static string EscapeJson(this string value)
    {
        ...
        if (value[0] == '$') return "__" + value.Substring(1);
    ...
    private static string UnescapeBson(this string value)
    {
        ...
        if (value[0] == '_' && value[1] == '_') return "$" + value.Substring(2);
