...
public class MongoGrainStorage : IGrainStorage, ILifecycleParticipant<ISiloLifecycle>
{
    ...
    private readonly ConcurrentDictionary<string, bool> configuredCollections = new();
    ...
    {
        mongoClient = mongoClientFactory.Create(options, "Storage");
        this.logger = logger;
        ...
        return DoAndLog(nameof(WriteStateAsync), async () =>
        {
            ...
            catch (MongoException ex)
            {
                if (ex.IsDuplicateKey())
                {
                    ...
                    try
                    {
                        await grainCollection.ReplaceOneAsync(Filter.Eq(FieldId, grainKey), document,
    ...
    protected virtual JsonSerializerSettings ReturnSerializerSettings(ITypeResolver typeResolver,
        IProviderRuntime providerRuntime, IProviderConfiguration config)
    {
        return OrleansJsonSerializer.UpdateSerializerSettings(
            OrleansJsonSerializer.GetDefaultSerializerSettings(typeResolver, providerRuntime.GrainFactory), config);
    }
    ...
    private static async Task ThrowForOtherEtag(IMongoCollection<BsonDocument> collection, string key, string etag,
        Exception ex)
    {
        var existingEtag =
    ...
    private async Task<T> DoAndLog<T>(string actionName, Func<Task<T>> action)
    {
        ...
        {
            logger.LogError((int) MongoProviderErrorCode.GrainStorageOperations, ex,
                $"{nameof(MongoGrainStorage)}.{actionName} failed. Exception={ex.Message}");
