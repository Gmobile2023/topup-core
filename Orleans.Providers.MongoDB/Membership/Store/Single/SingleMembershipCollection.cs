...
{
    private static readonly TableVersion NotFound = new(0, "0");
    private readonly string collectionPrefix;

    public SingleMembershipCollection(IMongoClient mongoClient, string databaseName, string collectionPrefix,
        bool createShardKey)
    ...
    public async Task<IList<Uri>> GetGateways(string deploymentId)
    {
        ...
        if (deployment == null) return new List<Uri>();

        return deployment.Members.Values.Where(x => x.Status == (int) SiloStatus.Active && x.ProxyPort > 0)
            .Select(x => x.ToGatewayUri()).ToList();
    }
    ...
    public async Task<MembershipTableData> ReadAll(string deploymentId)
    {
        ...
        if (deployment == null) return new MembershipTableData(NotFound);
        ...
        TableVersion tableVersion)
    {
        try
        {
            ...
            var etagCheck =
                etag == null ? Filter.Not(Filter.Exists(memberKey)) : Filter.Eq($"{memberKey}.Etag", etag);

            var result = await Collection.UpdateOneAsync(
                Filter.And(Filter.Eq(x => x.DeploymentId, deploymentId),
