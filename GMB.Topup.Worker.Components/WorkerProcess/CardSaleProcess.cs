...
namespace GMB.Topup.Worker.Components.WorkerProcess
{
    ...
    {
        public async Task<NewMessageResponseBase<List<CardRequestResponseDto>>> CardSaleRequest(
            WorkerPinCodeRequest request)
        {
            try
            {
                _logger.LogInformation("CardSaleRequest: " + request.ToJson());
                var response = new NewMessageResponseBase<List<CardRequestResponseDto>>();
        ...
        private async Task<NewMessageResponseBase<List<CardRequestResponseDto>>> CallCardGate(GateCardRequest request)
        {
            try
            {
        ...
        private async Task<(NewMessageResponseBase<List<CardRequestResponseDto>>, TransactionInfoDto)> CallCardPriority(
            ...
            List<ProviderConfig> providerCodes)
        {
            ...
            catch (Exception e)
            {
                ...
                var rs = new NewMessageResponseBase<List<CardRequestResponseDto>>
                {
                    ResponseStatus = new ResponseStatusApi(ResponseCodeConst.ResponseCode_WaitForResult,
                        "Giao dịch đang chờ kết quả. Vui lòng liên hệ CSKH để được hỗ trợ")
                };
