...
namespace HLS.Paygate.Worker.Components.WorkerProcess
{
    public partial class WorkerProcess
    {
        ...
        public async Task<NewMessageReponseBase<WorkerResult>> TopupRequest(WorkerTopupRequest request,
            ConsumeContext<TopupRequestCommand> context = null)
        {
            var slowTopup = false;
            const bool isRefund = true;
            var checkPhone = new NewMessageReponseBase<string>
            {
                ResponseStatus = new ResponseStatusApi("NOT_CHECK")
            };
            try
            {
                var response = new NewMessageReponseBase<WorkerResult>
                {
        ...
        private async ValueTask<NewMessageReponseBase<WorkerResult>> TopupAsync(WorkerTopupRequest request,
            NewMessageReponseBase<WorkerResult> response, SaleRequestDto saleRequest, bool isRefund,
        ...
        private async Task<NewMessageReponseBase<ResponseProvider>> CallTopupGate(GateTopupRequest topup,
            bool mustCount)
        {
            try
            {
        ...
        private async Task<(NewMessageReponseBase<ResponseProvider>, TransactionInfoDto)> CallTopupPriority(
            ...
            List<ProviderConfig> providerConfigs)
        {
            ...
            catch (Exception e)
            {
                ...
                var rs = new NewMessageReponseBase<ResponseProvider>
                {
                    ResponseStatus = new ResponseStatusApi(ResponseCodeConst.ResponseCode_WaitForResult,
                        "Giao dịch đang chờ kết quả. Vui lòng liên hệ CSKH để được hỗ trợ"),
                    Results = new ResponseProvider()
