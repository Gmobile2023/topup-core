...
public class Generator
{
    ...
    public Generator(short generatorId, DateTime start)
    {
        ...
        if (start > DateTime.Today) throw new ArgumentException("start date must not be in the future.", "start");
    ...
    public string TranCodeGenerator(string prefix = null)
    {
        return !string.IsNullOrEmpty(prefix) ? prefix + NextLong() : "P" + NextLong();
    }
    ...
    private void SpinToNextSequence()
    {
        ...
        while (time == _previousTime && _sequence >= _maxSequence) time = GetTime();
