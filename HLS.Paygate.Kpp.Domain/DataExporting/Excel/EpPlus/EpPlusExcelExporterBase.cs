...
namespace HLS.Paygate.Kpp.Domain.DataExporting.Excel.EpPlus;
...
public class EpPlusExcelExporterBase : IEpPlusExcelExporterBase
{
    ...
    public void AddObjectStartRowsIndex<T>(ISheet sheet,
        ...
        ICellStyle style, ICellStyle styleRed, IList<T> items,
        params Func<T, object>[] propertySelectors)
    {
        for (var i = 1; i <= items.Count; i++)
        {
            ...
            for (var j = 0; j < propertySelectors.Length; j++)
            {
                ...
                if (cellData.Format.ToLower() != "number")
                {
                    if ((string)cellData.Value == "Lệch")
    ...
    public void AddHeaderV2(ISheet sheet, int columnIndex, string headerText)
    {
