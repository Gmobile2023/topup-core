...
namespace HLS.Paygate.TopupGw.Components.Connectors.WhyPay;
                    ...
                    else
                    {
                        ...
                        else if (!string.IsNullOrEmpty(result.Status) && result.Status != "1")
                        {
                            if (result.Status == "0")
                            {
                                ...
                                responseMessage.ResponseCode = reResult != null ? reResult.ReponseCode : ResponseCodeConst.ResponseCode_ErrorProvider;
                                responseMessage.ResponseMessage = reResult != null ? reResult.ReponseName : "Giao dịch lỗi từ phía nhà cung cấp";
                            }
                            else
                            {
                                var reResult = await TopupGatewayService.GetResponseMassageCacheAsync(ProviderConst.WHYPAY, result.ResponseStatus, topupRequestLog.TransCode);
                                responseMessage.ResponseCode = reResult != null ? reResult.ReponseCode : ResponseCodeConst.ResponseCode_WaitForResult;
                                responseMessage.ResponseMessage = reResult != null ? reResult.ReponseName : result.Desc;
                            }
