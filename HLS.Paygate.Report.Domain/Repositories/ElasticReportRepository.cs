...
public partial class ElasticReportRepository : IElasticReportRepository
{
    ...
    public async Task<NewMessageReponseBase<ItemMobileCheckDto>> QueryMobileTransData(ReportRegisterInfo config, CheckMobileTransRequest request)
    {
        var dateStart = DateTime.Now;
        try
        {
            ...
            if (scanResults.Documents.Count > 0)
            {
                ...
                return new NewMessageReponseBase<ItemMobileCheckDto>()
                {
                    ResponseStatus = new ResponseStatusApi("01", "Thành công"),
                    Results = dataItem
                };
            }
            else return new NewMessageReponseBase<ItemMobileCheckDto>()
            {
