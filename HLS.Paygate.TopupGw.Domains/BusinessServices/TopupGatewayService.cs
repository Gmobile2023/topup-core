...
namespace HLS.Paygate.TopupGw.Domains.BusinessServices;
...
public class TopupGatewayService : BusinessServiceBase, ITopupGatewayService
{
    ...
    {
        var provider = request.ConvertTo<ProviderReponse>();
    ...
    {
        var providerReponse =
            await _transRepository.GetOneAsync<ProviderReponse>(p =>
        ...
        try
        {
            ...
            if (!string.IsNullOrEmpty(request.ReponseName) && providerReponse.ReponseName != request.ReponseName)
                providerReponse.ReponseName = request.ReponseName;

            if (!string.IsNullOrEmpty(request.ReponseCode) && providerReponse.ReponseCode != request.ReponseCode)
                providerReponse.ReponseCode = request.ReponseCode;
    ...
    public async Task<bool> ProviderResponseDeleteAsync(ProviderReponseDto request)
    {
        try
        {
            await _transRepository.DeleteOneAsync<ProviderReponse>(x =>
    ...
    public async Task<bool> ImportListProviderResponseAsync(ImportListProviderResponse request)
    {
        try
        {
            var listproviderResponse = request.ListProviderResponse.ConvertTo<List<ProviderReponse>>();
            await _transRepository.AddManyAsync<ProviderReponse>(listproviderResponse);
            return true;
        }
        catch (Exception ex)
        {
    ...
    public async Task<MessagePagedResponseBase> GetListResponseMessageAsync(GetListProviderResponse request)
    {
        try
        {
            Expression<Func<ProviderReponse, bool>> query = p => true;
            if (!string.IsNullOrEmpty(request.ResponseName))
            {
                Expression<Func<ProviderReponse, bool>> newQuery = p =>
                    p.ReponseCode.ToLower().Contains(request.ResponseName.ToLower());
            ...
            {
                Expression<Func<ProviderReponse, bool>> newQuery = p =>
                    p.ReponseCode == request.ResponseCode;
                query = query.And(newQuery);
            }
            ...
            var result = await _transRepository.GetSortedPaginatedAsync<ProviderReponse, Guid>(query,
    ...
    [DebuggerStepThrough]
    public async Task<ProviderReponse> GetResponseMassageCacheAsync(string provider, string code, string transcode)
    {
        try
        {
            var key = $"PayGate:ResponseMessageItems:{string.Join("_", provider, code)}";
            var response = await _cacheManager.GetEntity<ProviderReponse>(key);
            if (response != null) return response;
            response = await _transRepository.GetOneAsync<ProviderReponse>(c =>
