        ...
        private async Task CheckAutoClose(ProviderInfoDto provider)
        {
            try
            {
                ...
                if (provider.TotalTransErrorScan > 0 || provider.TotalTransDubious > 0)
                {
                    ...
                    var totalError = items.Count(p => p.Status == Contacts.Enums.TransRequestStatus.Fail);
                    var totalTimeout = items.Count(p => p.Status == Contacts.Enums.TransRequestStatus.Timeout);
